...
mlir::joinShapes(ArrayRef<int64_t> shape1, ArrayRef<int64_t> shape2,
                 Optional<Location> location) {
  if (shape1.size() != shape2.size())
    return emitOptionalError(location, "shapes' sizes differ");

  SmallVector<int64_t, 4> shape(shape1.size());
...
FailureOr<SmallVector<int64_t, 4>>
mlir::meetShapes(ArrayRef<int64_t> shape1, ArrayRef<int64_t> shape2,
                 Optional<Location> location) {
  if (shape1.size() != shape2.size())
    return emitOptionalError(location, "shapes' sizes differ");
