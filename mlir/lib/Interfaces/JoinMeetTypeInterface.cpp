...
Type mlir::joinTypes(Type ty1, Type ty2) {
  ...
  assert((!join1 || !join2 || join1.getValue() == join2.getValue()) &&
         "joinTypes commutativity was violated");

  return join1.hasValue() ? join1.getValue() : join2.getValueOr(Type());
}
...
Type mlir::meetTypes(Type ty1, Type ty2) {
  ...
  return meet1.hasValue() ? meet1.getValue() : meet2.getValueOr(Type());
}
